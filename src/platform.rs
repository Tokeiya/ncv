...
impl Platform {
	pub fn from_read(read: impl Read) -> Result<Platform, Error> {
		...
		let node = value
			.get("node")
			.ok_or(Error::EntryNotFound("node".to_string()))?;

		let tmp = node
			.get("runtime")
			.ok_or(Error::EntryNotFound("runtime".to_string()))?;
		let runtime = tmp
			.as_str()
			.ok_or(Error::UnexpectedValueType("Null".to_string()))?
			.to_string();

		let npm = node
			.get("npm")
			.ok_or(Error::EntryNotFound("npm".to_string()))?
			.as_str()
			.map(|x| x.to_string());

		let pnpm = value
			.get("pnpm")
			.ok_or(Error::EntryNotFound("pnpm".to_string()))?
			.as_str()
			.map(|x| x.to_string());

		let yarn = value
			.get("yarn")
			.ok_or(Error::EntryNotFound("yarn".to_string()))?
			.as_str()
			.map(|x| x.to_string());
...
mod test {
	...
	fn from_read_test() {
		let fixture =
			Platform::from_read(File::open("./test_artifacts/valid.json").unwrap()).unwrap();
	...
	fn invalid_format() {
		let fixture =
			Platform::from_read(File::open("./test_artifacts/invalid_format.txt").unwrap());
	...
	fn node_runtime_test() {
		let fixture =
			Platform::from_read(File::open("./test_artifacts/full_defined.json").unwrap()).unwrap();
